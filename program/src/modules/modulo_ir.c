...
uint32_t getTimeUs(void) { return Chip_TIMER_ReadCount(LPC_TIMER2); }

void resetTimeUs(void) { Chip_TIMER_Reset(LPC_TIMER2); }
...
void carrierCallback(void *ptr) {
  if (carrierEnabled) {
    ...
    gpioWrite(IR_TX_PIN,
              carrierPhase ? IR_ACTIVE_LEVEL : (1 - IR_ACTIVE_LEVEL));
  } else {
...
void irCarrierOn(void) {
  carrierEnabled = true;
  Timer_Init(0, Timer_microsecondsToTicks(CARRIER_HALF_US), carrierCallback);
}
...
void modulo_ir_send_nec(uint8_t addr, uint8_t cmd) {
  ...
  uint8_t cmdInv = ~cmd;
  uint32_t data = (uint32_t)addr | ((uint32_t)addrInv << 8) |
                  ((uint32_t)cmd << 16) | ((uint32_t)cmdInv << 24);
...
// Decodificador NEC
bool modulo_ir_decode(IRPulse_t *pulses, uint16_t count, uint8_t *address,
                      uint8_t *command) {
  if (count < 4)
    return false;
  if (!(pulses[0].level == 0 && pulses[0].duration > 8500 &&
        pulses[0].duration < 9500))
    return false;
  if (!(pulses[1].level == 1 && pulses[1].duration > 4000 &&
        pulses[1].duration < 5000))
    return false;

  uint32_t data = 0;
  for (int i = 2; i + 1 < count; i += 2) {
    if (!(pulses[i].level == 0 && pulses[i].duration > 400 &&
